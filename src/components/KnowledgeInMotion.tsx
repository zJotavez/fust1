  ...
  return (
    <section className="relative w-full py-20 sm:py-28 bg-fust-deep overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-20">
        ...
        {/* Puzzle/Quebra-Cabeça Grid of Blocks - Multi-column responsive gallery on mobile */}
        <div className="grid grid-cols-2 md:grid-cols-12 gap-4 auto-rows-[140px] md:auto-rows-[200px]">
          {puzzleItems.map((item, idx) => (
            <motion.div
              ...
              className={`${item.gridClass} group flex`}
            >
              {/* Inner card panel (relative overflow-hidden for background image overlays) */}
              <div className={`w-full flex flex-col h-full transition-all duration-500 ease-in-out shadow-lg hover:shadow-xl relative overflow-hidden rounded-lg ${item.cardClass}`}>
                ...
                {/* Photo Background (If defined, otherwise relies on glassmorphic backgrounds) */}
                {item.bgImage && (
                  <>
                    {/* Background image overlay with higher saturation and brightness */}
                    ...
                    {/* Shadow overlay gradient to guarantee contrast */}
                ...
                {/* Card Content (z-10 to stay on top of image backgrounds) */}
                <div className="space-y-2 sm:space-y-4 relative z-10">
                  ...
                  {/* Puzzle Block Title - Responsive sizes */}
                  <h3 className="font-serif text-xs sm:text-lg md:text-2xl font-bold tracking-tight leading-tight">
                    {item.title[lang]}
                  </h3>
                </div>

                {/* Puzzle Block Description - Responsive sizes */}
                <p className="text-[9px] sm:text-xs md:text-sm font-sans font-light leading-relaxed opacity-95 mt-1 sm:mt-4 relative z-10 line-clamp-4 md:line-clamp-none">
                  {item.desc[lang]}
                </p>
              </div>
            </motion.div>
